...
const handleSubmit = async () => {
  if (!token.value) {
    error.value = 'Le token de réinitialisation est requis'
    toastService.error('Échec de la réinitialisation', error.value)
    return
  }
  ...
  try {
    ...
    error.value = apiError.response?.data?.message || 'Impossible de réinitialiser le mot de passe. Veuillez réessayer.'
...
<template>
  <Card class="w-full max-w-md mx-auto">
    <CardHeader>
      <CardTitle class="text-2xl font-bold text-center">Réinitialiser le mot de passe</CardTitle>
    ...
    <CardContent>
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div v-if="!token" class="p-3 bg-red-50 text-red-700 rounded-md flex items-center gap-2">
          <AlertCircle class="h-5 w-5" />
          <span>Le token de réinitialisation est requis</span>
        </div>
